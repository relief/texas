...
def img2cv2gray(img):
	#img.show()
	print img.size
	#cv2_img = cv2.cv.CreateImageHeader(img.size,cv2.IPL_DEPTH_8U,3)    #CV Image
	cv2_data = numpy.array(img) 
	#cv2_data = cv2.cvtColor(cv2_data, cv2.cv.CV_RGBA2BGR)
	cv2_data = cv2.cvtColor(cv2_data, cv2.COLOR_RGB2GRAY)
	# cv2.cv.SetData(cv2_img, cv2_data)

	# cv2.cv.NamedWindow('cimg')
	# cv2.cv.ShowImage('cimg',cv2_img)
	# cv2.cv.WaitKey()
	return cv2_data
...
# im.save('screenshot.png')
	...
	def isChupai(self):
		...
		#region_state.save('state/tmp_chupai.png')
		return ic.compareChupai(img2cv2(region_state))

	# def isChupai(self):
	# 	region_state = self.im.crop(self.box_state)
		
	# 	return ic.compareChupai(region_state)
